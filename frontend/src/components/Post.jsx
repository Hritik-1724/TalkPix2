...
const Post = ({ post }) => {
    ...
    const likeOrDislikeHandler = async () => {
        try {
            ...
            const res = await apiClient.get(`/api/v1/post/${post._id}/${action}`);
    ...
    const commentHandler = async () => {
        ...
        try {
            const res = await apiClient.post(`/api/v1/post/${post._id}/comment`, { text }, {
                headers: {
    ...
    const deletePostHandler = async () => {
        try {
            const res = await apiClient.delete(`/api/v1/post/delete/${post?._id}`)
            if (res.data.success) {
    ...
    const bookmarkHandler = async () => {
        try {
            const res = await apiClient.get(`/api/v1/post/${post?._id}/bookmark`);
            if(res.data.success){
                toast.success(res.data.message);
            }
        } catch (error) {
